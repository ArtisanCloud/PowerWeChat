...
func (comp *Client) FileUpload(ctx context.Context, options *request.RequestWeDriveFileUpload) (*response.ResponseWeDriveFileUpload, error) {
	...
	dataMap["file_base64_content"] = options.FileBase64Content
	dataMap["file_name"] = options.FileName
	if options.SelectedTicket != "" {
		dataMap["selected_ticket"] = options.SelectedTicket
	} else {
		dataMap["spaceid"] = options.SpaceID
		dataMap["fatherid"] = options.FatherID
	}
	_, err := comp.BaseClient.HttpPostJson(ctx, "cgi-bin/wedrive/file_upload", dataMap, nil, nil, result)
